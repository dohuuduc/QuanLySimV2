...
namespace QuanLyData
{
    public partial class frmMain : Form
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            dm_Font dm_Font = SQLDatabase.Loaddm_Character("select * from dm_Font where isAct=1").FirstOrDefault();
            groupBox3.Text = string.Format("Dữ Liệu Nguồn (TXT: {0})", dm_Font.name);
                ...
                else
                {
                    ...
                    if (cbb_TypeDataSource.SelectedIndex == 0)
                    {
                        ...
                        else
                        {
                            ...
                            /*xử lý file ký tự đa75t biệt trước khi import vào*/
                            //https://stackoverflow.com/questions/13509532/how-to-find-and-replace-text-in-a-file-with-c-sharp
                            if (_cauhinh.isRemoveCharInFileImport)
                            {
                                string text = File.ReadAllText(txt_FileName.Text);
                                foreach (dm_Char item in SQLDatabase.Loaddm_Char("select * from dm_char"))
                                    text = text.Replace(Convert.ToChar(ConvertType.ToInt(item.Char)), Convert.ToChar(32));
                            ...
                            dm_Font dm_Font = SQLDatabase.Loaddm_Character(string.Format("select top 1 * from dm_Font where isAct=1")).FirstOrDefault();
                            ...
                            switch (sdef.DelimiterType)
                            {
                                case SchemaSpec.SchemeDef.DelimType.CsvDelimited:
                                    if (sdef.UsesHeader == SchemaSpec.SchemeDef.FirstRowHeader.Yes)
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=Yes;FMT=CsvDelimited;CharacterSet={0}""", dm_Font.ma);
                                    else
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=No;FMT=CsvDelimited;CharacterSet={0}""", dm_Font.ma);
                                    break;
                                case SchemaSpec.SchemeDef.DelimType.CustomDelimited:
                                    if (sdef.UsesHeader == SchemaSpec.SchemeDef.FirstRowHeader.Yes)
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + @";Extended Properties=""Text;HDR=Yes;CharacterSet=" + dm_Font.ma + ";FMT=Delimited(" + sdef.CustomDelimiter + ")" + "\"";
                                    else
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + @";Extended Properties=""Text;HDR=No;CharacterSet=" + dm_Font.ma + ";FMT=Delimited(" + sdef.CustomDelimiter + ")" + "\"";
                                    break;
                                case SchemaSpec.SchemeDef.DelimType.FixedWidth:
                                    if (sdef.UsesHeader == SchemaSpec.SchemeDef.FirstRowHeader.Yes)
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=Yes;CharacterSet={0};FMT=FixedLength""", dm_Font.ma);
                                    else
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=No;CharacterSet={0};FMT=FixedLength""", dm_Font.ma);
                                    break;
                                case SchemaSpec.SchemeDef.DelimType.TabDelimited:
                                    if (sdef.UsesHeader == SchemaSpec.SchemeDef.FirstRowHeader.Yes)
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=Yes;CharacterSet={0};FMT=TabDelimited""", dm_Font.ma);
                                    else
                                        connbit = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + path + string.Format(@";Extended Properties=""Text;HDR=No;CharacterSet={0};FMT=TabDelimited""", dm_Font.ma);
                                    break;
                                default:
                                    break;
                            }
        ...
        private RadioButton CreateRadio(int index, dm_column model)
        {
            ...
            radio.Text = string.Format("{0}_{1}", model.name, index+1);
            radio.Tag = model.ma;
            //radio.Height = 14;
        ...
        public void ProcessImport()
        {
            ...
            try
            {
                ...
                if (_cauhinh.DelImportTruocImport)
                ...
                */
                ...
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(SQLDatabase.ConnectionString))
                {
                    ...
                    finally
                    {
                        ...
                        reader.Close();
                        MessageBox.Show(string.Format("Hoàn tất import dữ liệu\n {0}", _cancelImport ? "Người dùng đã tạm dừng" : "", "Thông Báo"), "Thông Báo");
                        progressBar1.Invoke((Action)delegate
                        {
